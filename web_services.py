        ...
        for t in tasks:
            if isinstance(t,GroupOfOrders):
                for order in t.orders:
                    url = "https://priweb.mendelson.co.il/odata/Priority/tabula.ini/a121204/WTASKS('" + order.order_id + "')"
                    payload = json.dumps({
                        "DOERLOGIN": k
                    })
                    headers = {
    ...
    for k, tasks in schedule.items():
        ...
        for task in tasks:
            for item in task.lines:
                url = "https://priweb.mendelson.co.il/odata/Priority/tabula.ini/a121204/WTASKS('" + 11111 + "')"  # TODO BEN
